...
TEST(CppCP, fold) {
    ...
    auto &&r = fn(cbegin(target));
    EXPECT_EQ(r.get(), 6 + (target.at(0) + target.at(target.size() - 1)) *
                               (target.size() / 2.0));
...
TEST(CppCP, foldAndExitIfNullOpt) {
    ...
    auto &&fn = tig::cppcp::fold(
        tig::cppcp::sup<vitr<int>, int>(6),
        tig::cppcp::itr::any_unless_end<vitr<int>>(cend(target)),
        tig::cppcp::exit_if_nullopt<int, int>(
...
TEST(CppCP, reduce) {
    ...
    auto &&fn = tig::cppcp::reduce<vitr<int>, std::optional<int>>(
        ...
        [](auto &&a, auto &&v) {
            if(v) {
                return tig::cppcp::accm::contd(
                    std::make_optional(a.value() + v.value()));
            } else {
                return tig::cppcp::accm::terminate(a);
            }
        });
    auto &&r = fn(cbegin(target));
    EXPECT_EQ(r.get().value(), (target.at(0) + target.at(target.size() - 1)) *
                                   (target.size() / 2.0));
    EXPECT_EQ(r.itr(), cend(target));
}
...
TEST(CppCP, join2) {
    ...
    auto &&fn =
        join(an, fold(sup<vitr<int>, int>(6), itr::any_unless_end(cend(target)),
                      exit_if_nullopt<int, int>(
                          [](auto &&a, auto &&e) { return a + e; })));
    auto &&r = fn(cbegin(target));
    EXPECT_EQ(
        r.get(),
        std::make_tuple(-1, 6 + (target.at(1) + target.at(target.size() - 1)) *
                                    ((target.size() - 1) / 2.0)));
    EXPECT_EQ(r.itr(), cend(target));
}
...
TEST(CppCP, join3) {
    ...
    auto &&fn = join(itr::any<vitr<int>>(), itr::any<vitr<int>>(),
                     fold(sup<vitr<int>, int>(6),
    ...
    EXPECT_EQ(r.get(), std::make_tuple(
                           -2, -1,
                           6 + (target.at(2) + target.at(target.size() - 1)) *
                                   ((target.size() - 2) / 2.0)));
    EXPECT_EQ(r.itr(), cend(target));
}
...
template <class T> struct customSkip {
...
TEST(CppCP, join2AndSkipWithCustomSkip) {
    ...
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn =
        join_c<customSkip>::join(itr::any<vitr<int>>(), itr::any<vitr<int>>());
    auto &&r = fn(cbegin(target));
    EXPECT_EQ(r.get(), std::make_tuple());
    EXPECT_EQ(r.itr(), std::next(cbegin(target), 2));
}
TEST(CppCP, empty_tuple_as_skip_skipping) {
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn = empty_tuple_as_skip<vitr<int>>(
        join(itr::any<vitr<int>>(), itr::any<vitr<int>>()));
    auto &&r = fn(cbegin(target));
    EXPECT_EQ(r.itr(), std::next(cbegin(target), 2));
}
...
TEST(CppCP, trys_secondry_true) {
    ...
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto f = itr::any<vitr<int>>();
    auto &&fn = trys([](auto e) { return e >= 0; }, f,
                     map(f, [](auto &&e) { return e + 1; }));
    auto &&r = fn(cbegin(target));
    EXPECT_EQ(r.get(), 0);
    EXPECT_EQ(r.itr(), std::next(cbegin(target), 1));
}
...
TEST(CppCP, trys_all_false) {
    ...
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto f = itr::any<vitr<int>>();
    auto &&fn = trys([](const auto &e) { return false; }, f,
                     map(f, [](auto &&e) { return e + 1; }));
    EXPECT_THROW(fn(cbegin(target)),
                 tig::cppcp::all_of_parser_failed_exception);
}
...
TEST(CppCP, trys_variant_all_false) {
    ...
    auto f = itr::any<vitr<int>>();
    auto &&fn =
        trys_variant([](const auto &e) { return false; }, f,
                     map(f, [](auto &&e) { return std::to_string(e); }));
    EXPECT_THROW(fn(cbegin(target)),
                 tig::cppcp::all_of_parser_failed_exception);
}
TEST(CppCP, lazy) {
...
TEST(CppCP, catching) {
    ...
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn2 = make_catching<parser_exception>(
        throwing<vitr<int>, int>(parser_exception()),
...
TEST(CppCP, type_eraser) {
    ...
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn = type_eraser<vitr<int>, int>(
        throwing<vitr<int>, int>(parser_exception()));
    EXPECT_THROW(fn(cbegin(target)), tig::cppcp::parser_exception);
}
TEST(CppCP, many) {
    ...
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn = many(sup<vitr<int>>(std::vector<int>()), itr::any<vitr<int>>(),
                     [](auto &&list, auto v) {
                         list.push_back(v);
                         if(list.size() >= 4) {
...
TEST(CppCP, manyN) {
    ...
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn = manyN(sup<vitr<int>>(std::vector<int>()), itr::any<vitr<int>>(),
                      5, [](auto &&list, auto v) {
                          list.push_back(v);
                          if(list.size() >= 4) {
                              return accm::terminate(std::move(list));
...
TEST(CppCP, manyNM) {
    ...
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn = manyNM(sup<vitr<int>>(std::vector<int>()),
                       itr::any<vitr<int>>(), 5, 5, [](auto &&list, auto v) {
                           list.push_back(v);
                           if(list.size() >= 4) {
                               return accm::terminate(std::move(list));
                           }
...
TEST(CppCP, to_uncatching) {
    ...
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn = make_to_uncatching<parser_exception>(
        throwing<vitr<int>, int>(parser_exception()));
    EXPECT_THROW(fn(cbegin(target)).get(),
                 tig::cppcp::uncaught_parser_exception<parser_exception>);
}
TEST(CppCP, to_catching) {
    ...
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn =
        make_to_catching<parser_exception>(make_to_uncatching<parser_exception>(
            throwing<vitr<int>, int>(parser_exception())));
...
TEST(CppCP, state_machine) {
    using namespace std::literals::string_literals;
    using namespace tig::cppcp;
    std::vector<int> target{-1, 0, 1, 2, 3, 4, 5};
    auto &&fn = state_machine_parser(
        sup<vitr<int>, std::pair<std::vector<char>, std::vector<int>>>(
            std::pair<std::vector<char>, std::vector<int>>{
                std::vector<char>{'a'}, std::vector<int>()}),
        [](auto &&a, auto &&, auto &&e) {
            a.push_back(e);
            return accm::contd(a);
        },
